...
class CParser:
    ...
    def parse_directory(self, directory: str, target: str = None) -> None:
        """Parse all C files with multi-pass cross-file analysis."""
        
        # Inject target function so external/library calls are recognized
        if target:
            self.all_functions.add(target)
...
def analyze(project_dir: str, target: str = "mpf_mfs_open",
    ...
    parser.parse_directory(project_dir, target=target)
