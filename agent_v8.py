...
class CallGraphAnalyzer:
    ...
    def get_reachable_mfs_calls(self, verbose: bool = False) -> List[Dict]:
        """
        Find all mpf_mfs_open calls that are reachable from main().
        
        FIX 6: COMPLETE OVERHAUL - Search EVERY reachable function thoroughly:
        1. Check self.functions (extracted function bodies)
        2. Check self.macro_functions (macro bodies)
        3. Check self.prototypes via _search_prototype_definition
        4. FALLBACK: Search ALL files for ANY reachable function not found above
        5. Remove duplicates
        """
        calls = []
        found_functions = set()  # Track which functions we found mpf_mfs_open in
        searched_functions = set()  # Track which functions we actually searched
        
        if verbose:
            print(f"\n🔍 Searching {len(self.reachable_from_main)} reachable functions for mpf_mfs_open...")
        
        for func_name in self.reachable_from_main:
            func_searched = False
            
            # METHOD 1: Check in self.functions (extracted function bodies)
            if func_name in self.functions:
                func_searched = True
                searched_functions.add(func_name)
                func_data = self.functions[func_name]
                body = func_data['body']
                filepath = func_data['file']
                
                if verbose:
                    print(f"  ✓ {func_name}() - checking extracted body ({len(body)} chars)")
                
                # Find mpf_mfs_open calls in this function
                found_in_func = self._search_body_for_mfs(func_name, body, filepath, func_data['line'], is_macro=False)
                if found_in_func:
                    found_functions.add(func_name)
                    calls.extend(found_in_func)
                    if verbose:
                        print(f"    → Found {len(found_in_func)} mpf_mfs_open call(s)")
            
            # METHOD 2: Check in self.macro_functions (macro bodies)
            if func_name in self.macro_functions:
                ...
                if verbose:
                    print(f"  ✓ {func_name}() - checking macro body")
                ...
                if 'mpf_mfs_open' in body:
                    ...
                    if match:
                        ...
                        found_functions.add(func_name)
                        if verbose:
                            print(f"    → Found mpf_mfs_open in macro body")
            
            # METHOD 3: Check via prototype search (for functions with complex signatures)
            if func_name in self.prototypes:
                func_searched = True
                searched_functions.add(func_name)
                
                if verbose:
                    print(f"  ✓ {func_name}() - searching via prototype definition")
                ...
                for call_info in found_calls:
                    ...
                    call_info['search_method'] = 'prototype'
                    calls.append(call_info)
                    found_functions.add(func_name)
                
                if found_calls and verbose:
                    print(f"    → Found {len(found_calls)} mpf_mfs_open call(s)")
            
            # METHOD 4: FALLBACK - If function not in any category, search ALL files
            if not func_searched:
                if verbose:
                    print(f"  ⚠ {func_name}() - NOT in functions/macros/prototypes, using FALLBACK search")
                
                searched_functions.add(func_name)
                found_calls = self._fallback_search_function(func_name)
                for call_info in found_calls:
                    call_chain = self.get_call_chain(func_name)
                    call_info['call_chain'] = call_chain
                    call_info['reachable'] = True
                    call_info['search_method'] = 'fallback'
                    calls.append(call_info)
                    found_functions.add(func_name)
                
                if found_calls and verbose:
                    print(f"    → FALLBACK found {len(found_calls)} mpf_mfs_open call(s)")
        
        # METHOD 5: ADDITIONAL FALLBACK - Search ALL files for mpf_mfs_open and check if caller is reachable
        if verbose:
            print(f"\n🔍 Running additional fallback: scanning ALL files for mpf_mfs_open...")
        
        additional_calls = self._scan_all_files_for_mfs(verbose)
        for call_info in additional_calls:
            func_name = call_info.get('function')
            if func_name and func_name in self.reachable_from_main:
                # Check if we already found this call
                call_key = (call_info['filepath'], call_info['line'], call_info['function'])
                existing_keys = {(c['filepath'], c['line'], c['function']) for c in calls}
                
                if call_key not in existing_keys:
                    call_chain = self.get_call_chain(func_name)
                    call_info['call_chain'] = call_chain
                    call_info['reachable'] = True
                    call_info['search_method'] = 'full_scan'
                    calls.append(call_info)
                    found_functions.add(func_name)
                    if verbose:
                        print(f"    → FULL SCAN found: {func_name}() in {call_info['file']}")
        
        # Remove duplicates (same file, line, function)
        unique_calls = []
        seen = set()
        for call in calls:
            key = (call['filepath'], call['line'], call['function'])
            if key not in seen:
                seen.add(key)
        ...
        if verbose:
            print(f"\n📊 Summary:")
            print(f"   Reachable functions: {len(self.reachable_from_main)}")
            print(f"   Functions searched: {len(searched_functions)}")
            print(f"   Functions with mpf_mfs_open: {len(found_functions)}")
            print(f"   Total mpf_mfs_open calls: {len(unique_calls)}")
            
            # Show functions that were NOT searched
            not_searched = self.reachable_from_main - searched_functions
            if not_searched:
                print(f"   ⚠ Functions NOT searched: {sorted(not_searched)}")
        
        return unique_calls
    
    def _search_body_for_mfs(self, func_name: str, body: str, filepath: str, start_line: int, is_macro: bool = False) -> List[Dict]:
        """Search a function/macro body for mpf_mfs_open calls"""
        calls = []
        lines = body.split('\n')
    ...
    def _fallback_search_function(self, func_name: str) -> List[Dict]:
        """
        FALLBACK: Search ALL files for a function definition and mpf_mfs_open calls.
        Used when function is not in functions/macros/prototypes dictionaries.
        """
        calls = []
        ...
        for filepath, content in self.files.items():
            ...
            patterns = [
                # Standard: type func_name(params) {
                rf'\b{re.escape(func_name)}\s*\([^)]*\)\s*\{{',
                # With newline before {
                rf'\b{re.escape(func_name)}\s*\([^)]*\)\s*\n\s*\{{',
                # With attributes
                rf'__attribute__[^)]*\)\s*\b{re.escape(func_name)}\s*\([^)]*\)\s*\{{',
            ]
            ...
            for pattern in patterns:
                for match in re.finditer(pattern, content, re.MULTILINE | re.DOTALL):
                    ...
                    body_calls = self._search_body_for_mfs(func_name, func_body, filepath, func_start_line)
                    for call in body_calls:
                        call['search_method'] = 'fallback'
                    calls.extend(body_calls)
        
        return calls
    
    def _scan_all_files_for_mfs(self, verbose: bool = False) -> List[Dict]:
        """
        Scan ALL files for mpf_mfs_open calls and identify which function they're in.
        This is the ultimate fallback - finds calls even if function extraction failed.
        """
        calls = []
        
        for filepath, content in self.files.items():
            # Skip header files that are just declarations
            if filepath.endswith('.h') and 'mpf_mfs_open' in content:
                # Check if it's just a prototype
                lines = content.split('\n')
                for i, line in enumerate(lines):
                    if 'mpf_mfs_open' in line:
                        # Skip if it's a prototype (ends with ;) or comment
                        stripped = line.strip()
            ...
            # Find all mpf_mfs_open calls in .c files
            if filepath.endswith('.c') and 'mpf_mfs_open' in content:
                lines = content.split('\n')
                ...
                for i, line in enumerate(lines):
                    if 'mpf_mfs_open' in line:
                        ...
                        func_name = self._find_containing_function(content, i + 1)
    ...
    def _find_containing_function(self, content: str, line_num: int) -> Optional[str]:
        """Find which function contains a given line number"""
        lines = content.split('\n')
        
        # Search backwards from line_num to find function definition
        brace_depth = 0
        in_function = False
        func_name = None
        ...
        for i in range(line_num - 1, -1, -1):
            ...
            # If we've closed more braces than opened, we're inside a function
            if brace_depth < 0:
                in_function = True
            
            # Look for function definition
            if in_function or brace_depth <= 0:
                # Pattern: type func_name(params) { or type func_name(params)\n{
                match = re.search(r'\b(\w+)\s*\([^)]*\)\s*\{?\s*$', line)
                if match:
                    potential_name = match.group(1)
                    # Skip control flow keywords
                    if potential_name not in {'if', 'while', 'for', 'switch', 'else'}:
    ...
    def _search_prototype_definition(self, func_name: str) -> List[Dict]:
        """
        Search all files for the actual definition of a prototype function.
        ...
        for filepath, content in self.files.items():
            # Try multiple patterns to find the function definition
            patterns = [
                # Standard: func_name(params) {
                rf'\b{re.escape(func_name)}\s*\([^)]*\)\s*\{{',
                # With newline before {
                rf'\b{re.escape(func_name)}\s*\([^)]*\)\s*\n\s*\{{',
                # With attributes
                rf'__attribute__[^)]*\)\s*{re.escape(func_name)}\s*\([^)]*\)\s*\{{',
            ]
