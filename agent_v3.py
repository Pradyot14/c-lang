    ...
    def _build_call_graph(self):
        ...
        for func_name, func_data in self.functions.items():
            ...
            for match in re.finditer(r'\b([a-zA-Z_]\w*)\s*\(', clean):
                ...
                # Skip if preceded by type keyword (likely a declaration)
                pos = match.start()
                line_start = clean.rfind('\n', 0, pos) + 1
                before = clean[line_start:pos].strip()
                
                type_keywords = {'int', 'void', 'char', 'short', 'long', 'float', 'double',
                               'unsigned', 'signed', 'static', 'extern', 'const', 'struct', 'enum'}
                
                is_declaration = False
                if before:
                    words = before.split()
                    if words and words[-1].rstrip('*') in type_keywords:
                        is_declaration = True
                
                if not is_declaration:
                    # Add to call graph - either it's a known function or external
