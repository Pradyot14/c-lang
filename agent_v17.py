...
class CParser:
    ...
    def parse_macros(self, code: str, filename: str):
        """Parse macros - matches HTML logic"""
        # Handle line continuations
        code_exp = re.sub(r'\\\n', ' ', code)
        
        # Function-like: #define NAME(args) body
        # IMPORTANT: No space between NAME and ( - this distinguishes function-like from object-like
    ...
    def parse_functions(self, code: str, filename: str):
        ...
        for m in re.finditer(r'\b([a-zA-Z_]\w*)\s*\(([^)]*)\)\s*\{', clean):
            ...
            # Store function (prefer .c over .h)
            if name not in self.functions or filename.endswith('.c'):
                self.functions[name] = FunctionDef(name, filename, body, params, all_calls)
    
    # ==================== BUILD CALL GRAPH (matches HTML) ====================
    def build_graph(self):
        """Build call graph - matches HTML + callback detection"""
        # First, detect functions passed as arguments (callbacks)
        all_func_names = set(self.functions.keys())
        
        for func_name, func in self.functions.items():
            # Look for function names passed as arguments
            for call_match in re.finditer(r'\b(\w+)\s*\(([^)]+)\)', func.body):
                caller_func = call_match.group(1)
                args_str = call_match.group(2)
                for arg in args_str.split(','):
                    arg = arg.strip()
                    # If arg is a known function name, it's being passed as callback
                    if arg in all_func_names and arg != func_name:
                        func.calls[arg] = CallInfo(CallType.CALLBACK, via=caller_func)
...
class FullContextResolver:
    ...
    def _resolve_with_llm(self, mfs_call: MFSCallSite, max_iterations: int = 8) -> str:
        ...
        system_prompt = """You are a C code analyzer. Find the numeric value of the fileno argument.

The call chain shows each function, its parameters, and argument mappings.

STRATEGY:
1. Follow the BACKWARD TRACE - it shows the variable chain
2. If a variable is a PARAMETER, the caller's argument is shown
3. If a variable is a LOCAL VARIABLE, use get_variable_assignments()
4. For MACRO names (ALL_CAPS), use get_macro_value()
5. For arithmetic, use evaluate_expression()
6. Submit with submit_answer(values, trace)

Most cases are solved by following the pre-extracted chain."""
...
Follow the trace and resolve to numeric value(s)."""
            ...
            try:
                ...
                if msg.tool_calls:
                    for tc in msg.tool_calls:
                        ...
                        print(colored(f"      [{iteration+1}] {func_name}({func_args})", Colors.DIM))
                        
                        result = self.execute_tool(func_name, func_args)
                        
                        if result.startswith("ANSWER:"):
                            answer = result[7:]
                            print(colored(f"    ✓ LLM resolved: {answer}", Colors.GREEN))
                            return answer
                    ...
                    if msg.content:
                        numbers = re.findall(r'\b\d+\b', msg.content)
                        if numbers:
                            print(colored(f"    ✓ Extracted: {numbers[-1]}", Colors.GREEN))
                            return numbers[-1]
                    break
