...
class CallGraphAnalyzer:
    ...
    def _build_calls_for_macro(self, macro_name: str, macro_file: str, body: str):
        ...
        for match in re.finditer(r'(\w+)\s*\(', body):
            ...
            # Add if it's a known function or macro function
            if called in self.functions:
    ...
    def _find_reachable_from_main(self):
        """BFS from main() to find all reachable functions (including macro functions)"""
        ...
        while queue:
            ...
            # Add all functions/macros this one calls
            for called in self.call_graph.get(current, []):
                # FIX 3: Also consider macro functions as reachable targets
                if (called in self.functions or called in self.macro_functions) and called not in visited:
                    queue.append(called)
    ...
    def get_call_chain(self, target_func: str) -> List[str]:
        """Get the call chain from main() to target function (including macro functions)"""
        if 'main' not in self.functions:
            return []
        
        # FIX 3: Target can be a function or macro function
        if target_func not in self.functions and target_func not in self.macro_functions:
            return []
        ...
        while queue:
            ...
            for called in self.call_graph.get(current, []):
                ...
                if called in self.functions or called in self.macro_functions:
